...
class JobSearchLog(TimeStampedModel):
    ...
    location = models.CharField(
        max_length=255, blank=True, null=True, help_text="Localização da busca"
    )
    job_type = models.CharField(
        max_length=50, blank=True, null=True, help_text="Tipo de vaga (estágio, CLT, etc.)"
    )
    results_count = models.IntegerField(default=0, help_text="Número de resultados encontrados")
    filters = models.JSONField(default=dict, blank=True, help_text="Filtros aplicados na busca")
    results_preview = models.JSONField(
        default=list, blank=True, help_text="Preview dos primeiros resultados (máx 5)"
    )
