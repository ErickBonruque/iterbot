...
class SearchTermViewSet(viewsets.ModelViewSet):
    ...
    def by_course(self, request):
        ...
        if not course_id:
            return Response(
                {"error": "course_id é obrigatório"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class InteractionLogViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def clear(self, request):
        ...
        return Response(
            {"message": f"{count} log(s) de interação deletado(s) com sucesso", "count": count}
        )
...
class BotStatusViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        response_data = {**current_status, **metrics}
    ...
    def history(self, request):
        ...
        return Response({"checks": serializer.data, "period_hours": hours})
    ...
    def metrics(self, request):
        ...
        return Response(
            {"last_hour": metrics_1h, "last_24_hours": metrics_24h, "last_7_days": metrics_7d}
        )
