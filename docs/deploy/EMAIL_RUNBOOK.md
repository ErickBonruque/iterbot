# Email Runbook — v1.3 Resend Migration

Guia operacional para o provider de email Resend no IterBot UTFPR.
...
- [Rotação de RESEND_API_KEY](#rotação-de-resend_api_key)
...
| `EMAIL_FALLBACK_PROVIDER` | `ses` ou vazio | Provider de fallback automático |
| `RESEND_API_KEY` | Docker secret `resend_api_key` | Chave de API Resend |
